...
- **Money is the sun.** `BUDGET_USD_PER_MONTH` (300–500, default 400). Every token is written to `ledger/spend.jsonl`. The Steward moves everyone to cheaper models as the month runs low; at zero the town runs on habit alone — lore, not an outage.
...
- **Budget.** `BUDGET_USD_PER_MONTH` in `.env` (300–500). `config/budget.json` splits it — overseers 50 %, citizens 30 %, art 10 %, judge + verifier 10 % — and every ledger line carries its category, enforced before each call; the server derives a daily allowance from what is left in the month. Change it, then `sudo systemctl restart vesper`. An empty key means the town runs on habit alone.
